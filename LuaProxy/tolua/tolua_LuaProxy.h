...
//LuaEventHandler::handleAddImageAsync(const char *, LUA_FUNCTION func)
static int tolua_LuaEventHandler_handleAddImageAsync(lua_State *l){
#ifndef TOLUA_RELEASE
	tolua_Error err;
	if(!tolua_isusertype(l, 1, "LuaEventHandler", 0, &err) || !tolua_isstring(l, 2, 0, &err)){
		tolua_error(l,"#ferror in function 'LuaEventHandler.handleAddImageAsync'.",&err);
		return 0;
	}
#endif
	LuaEventHandler *h = (LuaEventHandler *)tolua_tousertype(l, 1, NULL);
	if(h){
		h->handleAddImageAsync(tolua_tostring(l, 2, NULL), toluafix_ref_function(l, 3, 0));
	}
	tolua_pushusertype(l, h, "LuaEventHandler");
	return 1;
}
